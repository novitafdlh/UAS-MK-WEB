    ...
    <div class="mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="text-gray-600">Kelola data akun mahasiswa dengan mudah</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('mahasiswa.akun.create') }}" 
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-rose-100 p-6 hover:shadow-lg transition-shadow duration-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-rose-600">{{ $akuns->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-rose-100 p-6 hover:shadow-lg transition-shadow duration-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600">{{ $akuns->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-rose-100 p-6 hover:shadow-lg transition-shadow duration-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-blue-600">{{ $akuns->pluck('prodi_id')->unique()->count() }}</p>
                </div>
                        ...
                        <tr class="hover:bg-gradient-to-r hover:from-rose-50 hover:to-red-50 transition-all duration-200">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <div class="font-medium text-gray-900">{{ $akun->nim }}</div>
                                        <div class="text-xs text-gray-500">Mahasiswa</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $akun->nama }}</div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    <a href="{{ route('mahasiswa.akun.edit', $akun->id) }}" 
                                    ...
                                    <form action="{{ route('mahasiswa.akun.destroy', $akun->id) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
                                                onclick="return confirm('Yakin ingin menghapus akun {{ $akun->nama }}? Data yang dihapus tidak dapat dikembalikan.')"
            ...
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-600">
                        Menampilkan <span class="font-medium">{{ $akuns->count() }}</span> dari <span class="font-medium">{{ $akuns->count() }}</span> data
                    </div>
                    <div class="text-sm text-gray-500">
                        Total {{ $akuns->count() }} akun mahasiswa terdaftar
                    </div>
                </div>
            </div>
        @else
            <div class="text-center py-16">
                ...
                <h3 class="text-xl font-semibold text-gray-900 mb-2">Belum Ada Data Akun</h3>
                <p class="text-gray-500 mb-6 max-w-md mx-auto">
                    Belum ada akun mahasiswa yang terdaftar dalam sistem. Mulai dengan menambahkan akun mahasiswa pertama.
                </p>
                <a href="{{ route('mahasiswa.akun.create') }}" 
                   class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-rose-500 to-red-500 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl hover:from-rose-600 hover:to-red-600 transform hover:-translate-y-0.5 transition-all duration-200">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
