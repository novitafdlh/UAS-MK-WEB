...
<div class="max-w-6xl mx-auto px-4 py-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Daftar Matakuliah</h1>
        ...
        <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-5 shadow relative">
            ...
            <button type="button" class="absolute top-0 bottom-0 right-0 px-4 py-3" onclick="this.parentElement.remove();">
                <svg class="fill-current h-6 w-6 text-green-500" viewBox="0 0 20 20">
                    <path d="M14.348 14.849a1.2 1.2 0 01-1.697 0L10 11.819l-2.651 3.03a1.2 1.2 0 01-1.697-1.698l2.758-3.15
                             -2.758-3.15A1.2 1.2 0 017.349 5.12L10 8.183l2.651-3.063a1.2 1.2 0 111.697 1.697l-2.758 3.15
                             2.758 3.15a1.2 1.2 0 010 1.698z"/>
                </svg>
            </button>
        </div>
    @endif
    ...
    <div class="overflow-x-auto bg-white border rounded-xl shadow">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-red-100 text-red-900">
                <tr>
                    <th class="px-5 py-3 text-left text-xs font-semibold uppercase">Kode</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold uppercase">Nama</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold uppercase">SKS</th>
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                ...
                <tr class="hover:bg-red-50 transition">
                    <td class="px-5 py-3 text-sm text-gray-800">{{ $matakuliah->kode }}</td>
                    <td class="px-5 py-3 text-sm text-gray-800">{{ $matakuliah->nama }}</td>
                    <td class="px-5 py-3 text-sm text-gray-800">{{ $matakuliah->sks }}</td>
                    <td class="px-5 py-3 text-sm text-gray-800">{{ $matakuliah->jurusan }}</td>
                    <td class="px-5 py-3 text-sm text-center">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            {{-- Tombol Hapus --}}
                            <form action="{{ route('admin.matakuliah.destroy', $matakuliah->id) }}" method="POST"
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-5 py-6 text-center text-gray-500 italic">😕 Belum ada data matakuliah.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
