...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.dosen.create') }}"
           class="inline-block px-6 py-2 bg-gradient-to-r from-dark-blue-500 to-dark-blue-800 text-white rounded-lg shadow hover:scale-105 transition-all">
            + Tambah Dosen
        </a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow p-6 overflow-x-auto">
        <table class="min-w-full table-auto">
            <thead>
                <tr class="bg-dark-blue-100 text-gray-700">
            ...
            <tbody>
                @forelse($dosen as $dsn)
                    <tr class="border-b hover:bg-dark-blue-50">
                        ...
                        <td class="px-4 py-2 flex gap-2">
                            <a href="{{ route('admin.dosen.edit', $dsn->id) }}"
                               class="px-3 py-1 bg-red-accent-400 text-white rounded hover:bg-red-accent-500 text-sm">Edit</a>
                            <form action="{{ route('admin.dosen.destroy', $dsn->id) }}" method="POST" onsubmit="return confirm('Yakin hapus?')" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="px-3 py-1 bg-red-accent-600 text-white rounded hover:bg-red-accent-700 text-sm">Hapus</button>
                            </form>
                        </td>
                    </tr>
                @empty
