...
<main class="flex-1 p-4 md:p-6 bg-gradient-to-br from-rose-50 to-red-50 min-h-screen">
        ...
        <div class="flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-rose-500 font-medium">Kelola akun dosen dengan mudah</p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.dosen.akun.create') }}" 
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <div class="group relative bg-white rounded-2xl shadow-sm border border-rose-100 p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
            ...
            <h3 class="text-lg font-semibold text-gray-700 mb-2">Total Akun Dosen</h3>
            <p class="text-3xl font-bold text-red-600 mb-1">{{ $dosenUsers->count() }}</p>
            <p class="text-sm text-gray-500">Akun dosen aktif</p>
        </div>
    </div>
        ...
        <div class="p-6 md:p-8">
            @if ($dosenUsers->isEmpty())
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 text-lg font-medium">Belum ada akun dosen</p>
                    <p class="text-gray-400 text-sm mt-1">Tambahkan akun dosen pertama Anda</p>
                </div>
            @else
                    ...
                    <tbody class="divide-y divide-rose-100">
                        @foreach($dosenUsers as $index => $user)
                            <tr class="hover:bg-rose-50 transition-colors duration-200">
                                <td class="px-4 py-3">
                                    <div class="w-8 h-8 bg-gradient-to-br from-red-100 to-rose-100 rounded-lg flex items-center justify-center">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('admin.dosen.akun.edit', $user->id) }}" 
                                        ...
                                        <form action="{{ route('admin.dosen.akun.destroy', $user->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus akun ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
