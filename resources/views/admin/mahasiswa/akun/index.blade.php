...
@section('content')
<main class="flex-1 p-4 md:p-6 bg-gradient-to-br from-white min-h-screen">
    {{-- Header --}}
    <header class="bg-white rounded-2xl shadow-sm border border-rose-100 p-6 mb-8">
        <div class="flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text ">
                        Daftar Akun Mahasiswa
                    </h1>
                    <p class="text-sm text-rose-500 font-medium">Kelola akun mahasiswa dengan mudah</p>
                </div>
            </div>
    ...
    <div class="text-center mb-8">
        <h2 class="text-3xl md:text-4xl font-bold bg-gradient-to-r from-red-300 to-red-600 bg-clip-text ">
            Manajemen Akun Mahasiswa
        </h2>
        <div class="w-24 h-1 bg-gradient-to-r from-red-300 to-red-600 mx-auto rounded-full"></div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <div class="group bg-white rounded-2xl shadow-sm border border-rose-100 p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
            <div class="flex justify-end">
                <div class="w-12 h-12 bg-gradient-to-br from-red-300 to-red-600 rounded-xl flex items-center justify-center shadow-lg">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                    </svg>
                </div>
            </div>
            <h3 class="text-lg font-semibold text-black mb-2">Total Akun</h3>
            <p class="text-3xl font-bold text-black mb-1">{{ $mahasiswaUsers->count() }}</p>
            <p class="text-sm text-black">Akun mahasiswa</p>
        </div>
    </div>

    {{-- Tabel Akun --}}
    <div class="bg-white rounded-2xl shadow-sm border border-rose-100 overflow-hidden">
        <div class="bg-gradient-to-r from-white to-white p-6 text-black">
        ...
        <div class="p-6 md:p-8">
            @if ($mahasiswaUsers->isEmpty())
                <div class="text-center py-12 ">
                    ...
                    <p class="text-gray-500 text-lg font-medium">Belum ada akun mahasiswa</p>
                    <p class="text-gray-400 text-sm mt-1">Tambahkan akun mahasiswa pertama Anda</p>
                </div>
            @else
                    ...
                    <tbody class="divide-y divide-rose-100">
                        @foreach($mahasiswaUsers as $index => $user)
                            <tr class="hover:bg-rose-50 transition-colors duration-200">
                                <td class="px-4 py-3">
                                    <div class="w-8 h-8 bg-gradient-to-br from-pink-100 to-rose-100 rounded-lg flex items-center justify-center">
                                        <span class="text-sm font-bold text-rose-600">{{ $index + 1 }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 bg-gradient-to-br from-rose-100 to-pink-100 rounded-xl flex items-center justify-center">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('admin.mahasiswa.akun.edit', $user->id) }}" 
                                           class="inline-flex items-center px-3 py-2 bg-gradient-to-r from-red-300 to-red-600 text-white text-xs font-medium rounded-lg hover:from-red-600 hover:to-red-300 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-200">
                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                            </svg>
                                            Edit
                                        </a>
                                        <form action="{{ route('admin.mahasiswa.akun.destroy', $user->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus akun ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
                                                    class="inline-flex items-center px-3 py-2 bg-gradient-to-r from-red-300 to-red-600 text-white text-xs font-medium rounded-lg hover:from-red-600 hover:to-red-300 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-200">
