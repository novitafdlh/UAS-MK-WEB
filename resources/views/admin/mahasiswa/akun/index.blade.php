...
<div class="max-w-6xl mx-auto px-4 py-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Daftar Akun Mahasiswa</h1>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                ...
                <tr class="hover:bg-red-50 transition">
                    <td class="px-5 py-3 text-sm text-gray-800">{{ $user->name }}</td>
                    <td class="px-5 py-3 text-sm text-gray-800">{{ $user->email }}</td>
                    <td class="px-5 py-3 text-sm text-gray-800">{{ $user->getRoleNames()->first() ?? 'Mahasiswa' }}</td>
                    <td class="px-3 py-3 text-sm text-center">
                        <div class="flex items-center justify-center gap-1">
                            {{-- Tombol Edit --}}
                            <a href="{{ route('admin.mahasiswa.akun.edit', $user->id) }}"
                               class="inline-flex items-center px-3 py-1 text-xs font-medium text-white bg-yellow-500 hover:bg-yellow-600 rounded shadow">
                            ...
                            {{-- Tombol Hapus --}}
                            <form action="{{ route('admin.mahasiswa.akun.destroy', $user->id) }}" method="POST"
                                  onsubmit="return confirm('Yakin ingin menghapus akun ini?');" class="m-0 p-0">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                        class="inline-flex items-center px-3 py-1 text-xs font-medium text-white bg-red-600 hover:bg-red-700 rounded shadow">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-5 py-6 text-center text-gray-500 italic">😕 Belum ada akun mahasiswa.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
