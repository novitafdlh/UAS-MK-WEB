...
<main class="flex-1 p-4 md:p-6 bg-gradient-to-br from-dark-blue-50 to-gray-50 min-h-screen">
    ...
    <div class="max-w-xl mx-auto bg-white p-8 rounded-xl shadow-md border border-dark-blue-100">
        ...
        @if(session('success'))
            <div class="bg-dark-blue-100 border border-dark-blue-400 text-dark-blue-700 px-4 py-3 rounded-xl mb-6 shadow-sm">
                {{ session('success') }}
            </div>
        @endif

        <form action="{{ route('dosen.akun.update') }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
